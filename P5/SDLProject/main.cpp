...
void Initialize() {
    ...
    music = Mix_LoadMUS("background_music.wav");
...
void ProcessInput() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYUP:
            ...
            case SDLK_RIGHT:
                currentScene->state.player->animIndices = currentScene->state.player->animIdleRight;
                break;
            }
        }
    }
...
void Update() {
    ...
    if (currentScene->state.player->hit) {
        ...
        if (lives <= 0) {
            ...
            currentScene->state.player->position = glm::vec3(1.5f, -3, 0);
        }
    }

    //move camera opposite to player, move up into view
    viewMatrix = glm::mat4(1.0f);
    if (currentScene->state.player->position.x > 5 && currentScene->state.player->position.x < 17 && currentScene != sceneList[0]) {
        xHUD = currentScene->state.player->position.x;
    }
    else if (currentScene->state.player->position.x > 17) {
        xHUD = 17;
    }
    else {
        xHUD = 5;
    ...
    viewMatrix = glm::translate(viewMatrix, glm::vec3(-xHUD, 3.75, 0));
...
void Render() {
    ...
    if (currentScene != sceneList[0]) {
        Util::DrawText(&program, fontTextureID, "Lives: " + std::to_string(lives), 0.5f, -0.25f, glm::vec3(xHUD - 4.5f, -0.5f, 0));

        if (currentScene->state.player->win) {
            Util::DrawText(&program, fontTextureID, "You Win!", 0.75f, -0.25f, glm::vec3(xHUD - 1.75, -3.0f, 0));
        }
        else if (currentScene->state.player->lose) {
            Util::DrawText(&program, fontTextureID, "You Lose!", 0.75f, -0.25f, glm::vec3(xHUD - 2, -3.5f, 0));
